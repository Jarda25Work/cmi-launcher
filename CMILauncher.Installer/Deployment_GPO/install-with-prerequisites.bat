...
REM Zjistit cestu ke skriptu
set SCRIPT_DIR=%~dp0
if not exist "%SCRIPT_DIR%CMILauncherSetup.msi" (
    echo CHYBA: MSI soubor nebyl nalezen: %SCRIPT_DIR%CMILauncherSetup.msi
    echo Ujistete se, ze CMILauncherSetup.msi je ve stejne slozce jako tento skript.
    pause
    exit /b 1
)

echo   → Spoustim instalaci MSI...
msiexec /i "%SCRIPT_DIR%CMILauncherSetup.msi" /qb
if %errorLevel% neq 0 (
    echo CHYBA: Instalace CMI Launcher selhala s kodem: %errorLevel%
    pause
    exit /b 1
)
...
echo ================================================
echo.
echo POZNAMKA: Pokud jste prave nainstalovali WebView2 Runtime,
echo           doporucujeme RESTARTOVAT pocitac pred prvnim spustenim
echo           CMI Launcher aplikace.
echo.
pause
